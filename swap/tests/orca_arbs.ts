...
async function pool_swap_ix(
  ...
  console.log(
    `setting up swap for ... ${inputToken["tag"]} -> ${outputToken["tag"]}`
  );
...
describe("tmp", async () => {
  ...
  var pool_path =
    "/Users/brennan/Documents/workspace/solana/orca_local/env/pools/params_AB.json";
  const AB_pool_params = decode_poolparams(pool_path);
  var pool_path =
    "/Users/brennan/Documents/workspace/solana/orca_local/env/pools/params_BC.json";
  const BC_pool_params = decode_poolparams(pool_path);
  var pool_path =
    "/Users/brennan/Documents/workspace/solana/orca_local/env/pools/params_CA.json";
  const CA_pool_params = decode_poolparams(pool_path);

  const pools = [AB_pool_params, BC_pool_params, CA_pool_params];
  let name2token = {};
    ...
    for (let j = i; j < n_tokens; j++) {
      if (i == j) {
        ...
        if (!(id in token2pool)) {
          ...
          let quote: orca_root.Quote = await pool.getQuote(
            tokenA,
            new Decimal(1)
          );
  ...
  for (i = 0; i < arb_path.length - 1; i++) {
    ...
    console.log(
      `\t EXCHANGE RATE: 1 ${token0_name} = ${ex_rate} ${token1_name}`
    );
  ...
  var init_token_account_addr = await orca_root.deriveAssociatedTokenAddress(
    owner.publicKey,
    init_token.mint
  );
  var _start_balance =
    (await get_balance(connection, init_token_account_addr)) *
    10 ** init_token.scale;
  ...
  const recentBlockHash = (await connection.getRecentBlockhash("singleGossip"))
    .blockhash;
  const txFields: web3.TransactionCtorFields = {
    recentBlockhash: recentBlockHash,
    feePayer: owner.publicKey,
  };
  const transaction = new web3.Transaction(txFields).add(...ixs);
